...
import { BookOpen, Trash2, Search, Loader2, ArrowLeft, AlertCircle, Eye, FileText, Activity, RefreshCw, ChevronRight } from 'lucide-react';
...
export default function ManageTests() {
  ...
  const fetchMocks = async () => {
    ...
    try {
      ...
      // Fixing potential data structure issues
  ...
  const handleDelete = async (mockName) => {
    if (!window.confirm(`⚠️ Permanently Delete "${mockName}"?`)) return;
    ...
    try {
      ...
      if (res.ok) setMocks(prev => prev.filter(m => (m.name || m) !== mockName));
    } catch (err) {
        ...
        {filteredMocks.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
            {filteredMocks.map((mock) => {
              ...
              return (
                <div key={mockName} className="group relative bg-zinc-900/30 border border-zinc-800/50 p-8 rounded-[2.5rem] hover:bg-zinc-900/50 hover:border-blue-500/40 transition-all duration-500 shadow-2xl backdrop-blur-sm overflow-hidden">
                  ...
                  <div className="flex justify-between items-start mb-8">
                    ...
                    <button 
                      onClick={() => handleDelete(mockName)} 
                      className="text-zinc-700 hover:text-red-500 p-2 transition-all hover:bg-red-500/10 rounded-xl"
                    >
                      {deletingName === mockName ? <Loader2 size={20} className="animate-spin" /> : <Trash2 size={20} />}
                    </button>
                  </div>

                  <h3 className="text-2xl font-black text-white mb-3 uppercase tracking-tighter group-hover:text-blue-400 transition-colors">{mockName}</h3>
                  
                  <div className="flex items-center gap-3 mb-8">
                    <div className="flex h-2 w-2 rounded-full bg-emerald-500 animate-pulse"></div>
                    <span className="text-zinc-500 text-[10px] font-black uppercase tracking-[0.2em]">{mockCount} Questions Registered</span>
                  </div>

                  {/* 🔥 REDIRECT LOGIC BUTTON */}
                  <button 
                    onClick={() => {
                      // encodeURIComponent taaki space aur special chars URL mein break na karein
                      router.push(`/admin/upload?edit=${encodeURIComponent(mockName)}`);
                    }}
                    className="w-full py-4 bg-zinc-800 hover:bg-blue-600 text-white rounded-2xl font-black text-[10px] tracking-[0.3em] uppercase transition-all flex items-center justify-center gap-2 group/btn shadow-xl shadow-black/20"
                  >
                    Enter Control Room <ChevronRight size={14} className="group-hover/btn:translate-x-1 transition-transform" />
                  </button>
                </div>
              );
            })}
